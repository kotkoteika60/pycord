...
const yabbcode = function(config = {}){
    ...
    this.tags = {
        ...
        'code': {
            type: 'content',
            replace: (attr, content) => `<div class="hljs codeblock"><code class="hljs ${attr || ''}" style="">${content || ''}</code></div>`,
        },
        'attachment': {
            type: 'content',
            replace: (attr, content) => `<div class="attachment"><a href="/static/attachments/${attr}" target="_blank">${content}</a>[вложение]</div>`,
        },
        'reply': {
            type: 'content',
            replace: (attr, content) => `<a href="#M_${attr}" onclick="msgFocus(${attr})">${content}</a>`,
        },
        'c': {
            type: 'replace',
            open: attr => `<span style="color: ${attr || 'white'}">`,
            close: '</span>',
        },
        'color': {
            type: 'replace',
